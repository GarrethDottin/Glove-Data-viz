...
def read(f):
    ...
    for l in f:
        ...
        m[w] = v
    if errorCounter >0:
       logger("glove datafile had unicode problems",errorCounter,"lines dropped.")
    return m, words, d

#Merely adding the features and words to the annoy index
def buildAnnoyIndex(model, words, dim):
    ...
    for i,w in enumerate(words):
        ...
        if i>0 and (i-1)*10/n != i*10/n:
            logger("Annoy index building at", i*100/n, "percent.")
    end()
...
def queryAnnoyIndex(model, annoyIndex, words, dim, n):
    ...
    for i,w in enumerate(words):
        ...
        if i>0 and (i-1)*10/n != i*10/n:
               logger("Annoy index querying at", i*100/n, "percent.")
    end()
    logger("Minimal quality evaluation:", float(badSign)/n)
